...
export default async function AdminTeamsPage() {
  ...
  if (viewAsId) {
    const { data: viewAsEmp } = await db
      .from('employees')
      .select('id, auth_user_id, name, email, role, employment_type, hire_date, avatar_url, instagram_url, status, requested_team_id, line_user_id, notifications_read_at, created_at, updated_at')
      .eq('id', viewAsId)
      .single()
    if (viewAsEmp) {
