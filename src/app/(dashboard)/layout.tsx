  ...
  if (!employee) {
    ...
    if (!insertError) {
      ...
      const { data: created } = await adminDb
        .from('employees')
        .select('id, name, email, role, employment_type, hire_date, avatar_url, instagram_url, status, requested_team_id, requested_project_team_id, line_user_id, notifications_read_at, auth_user_id, created_at, updated_at')
