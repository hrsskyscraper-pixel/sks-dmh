...
export default async function DashboardPage({
  ...
  const [targetEmployeeResult, params] = await Promise.all([
    viewAsId
      ? db.from('employees').select('id, name, last_name, first_name, name_kana, email, role, business_role_ids, system_permission, employment_type, hire_date, birth_date, avatar_url, instagram_url, line_url, line_user_id, line_friend, status, requested_team_id, requested_project_team_id, approved_by, approved_at, notifications_read_at, font_scale, intro_dismissed_at, is_test, auth_user_id, created_at, updated_at').eq('id', viewAsId).single()
  ...
  const [
    ...
    db.from('achievements').select('id, skill_id, employee_id, status, achieved_at, certified_by, certified_at, cumulative_hours_at_achievement, notes, apply_comment, certify_comment, is_read, photo_paths, created_at, skills(id, name, phase, category, order_index, target_date_hint, standard_hours, is_checkpoint, created_at), certified_employee:employees!achievements_certified_by_fkey(name, avatar_url)').eq('employee_id', employee.id),
  ...
  return (
    <>
      ...
      />
      {currentEmployee.line_friend !== true && <LineLinkBanner needsFriend={!!currentEmployee.line_user_id} />}
      <DashboardContent
        employee={employee}
        skills={skills}
