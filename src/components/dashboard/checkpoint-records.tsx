              ...
              return (
                <div key={skill.id} className={`rounded-lg px-3 py-2.5 ${isAchieved ? 'bg-green-50 border border-green-200' : 'bg-gray-50 border border-gray-100'}`}>
                  <div className="flex items-center gap-2 mb-1.5">
                    <Badge className="text-[9px] bg-red-500 text-white border-0 flex-shrink-0 px-1.5">CP</Badge>
                        ...
                        <div key={rank} className="flex items-center gap-1 flex-1 min-w-0" title={entry ? `${holder?.name ?? '不明'} ${entry.hours}h` : ''}>
                          {entry ? (
                            <>
                              ...
                              <span className={`text-xs truncate ${isMe ? 'font-bold text-orange-600' : 'text-gray-700'}`}>{holder?.name?.split(/\s/)[0] ?? '?'}</span>
